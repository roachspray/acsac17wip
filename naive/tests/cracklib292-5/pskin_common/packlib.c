...
PWDICT *
PWOpen(in_pwi, in_pwd, in_hwm, mode)
    const char *in_pwi;
    const char *in_pwd;
    const char *in_hwm;
    char *mode;
{
    ...
    snprintf(iname, STRINGSIZE, "%s", in_pwi);
    snprintf(dname, STRINGSIZE, "%s", in_pwd);
    snprintf(wname, STRINGSIZE, "%s", in_hwm);
    ...
    if (mode[0] == 'r')
    {
		...
		if (!(pdesc.dfp = fopen(dname, mode)))
		{
			...
			pdesc.flags |= PFOR_USEZLIB;
			/* try extension .gz */
			snprintf(dname, STRINGSIZE, "%s.pwd.gz", in_pwi);
